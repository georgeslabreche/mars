...
sunrise = function(Ls, phi, unit=1, beta=NULL, gamma_c=NULL){
  ...
  polar_flag = -tan(delta_rad) * tan(phi_rad)
  ...
  if((is.null(beta) && is.null(gamma_c))){
    omega_rad = sunrise_for_horizontal_surface(phi=phi_rad, delta=delta_rad, unit=unit)
    ...
    # Inclination angle is 0 degrees, this is equivalent to a horizontal surface.
      ...
      omega_rad = sunrise_for_inclined_surface_oriented_east(phi=phi_rad, beta=beta*pi/180, gamma_c=gamma_c*pi/180, delta=delta_rad)
      
    }else if(gamma_c > 0){ # Incline surface is oriented towards the West.
      omega_rad = sunrise_for_inclined_surface_oriented_west(phi=phi_rad, beta=beta*pi/180, gamma_c=gamma_c*pi/180, delta=delta_rad)
    }
