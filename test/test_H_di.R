...
Hdh_beta_eq = dget(here("functions", "H_dh_beta.R"))
...
expect_equal_all = function(tolerance, Ls_seq, phi, longitude, beta, measured_taus, expected_insolations){
  for(Ls in Ls_seq){
    ...
    Hdh_beta_expected = expected_insolations[expected_insolations$Ls == Ls, ]$Hd
    
    # Calculated Hdh on inclined surface.
    Hdh_beta_calculated = Hdh_beta_eq(Ls=Ls, phi=phi, longitude=longitude, tau=tau_measured, beta=beta, gamma_c=gamma_c, nfft=nfft)
    Hdh_beta_calculated = round(Hdh_beta_calculated, 1)
    
    print(paste("Ls", Ls, "tau", tau_measured, "Hdh_c:", Hdh_beta_calculated, "Hdh_e:", Hdh_beta_expected, "diff:", (Hdh_beta_calculated-Hdh_beta_expected)))

    # Assert equality.
    expect_equal(Hdh_beta_calculated, Hdh_beta_expected, tolerance=tolerance, scale=1)
  }  
}


test_that("H_dh_beta: Diffuse daily insolation on an inclined surface for beta = phi at VL1.", {
...
# test_that("H_dh_beta: Diffuse daily insolation on an inclined surface for beta = phi at VL2.", {
# 
#   tolerance = 0.5
# 
#   Ls_seq = seq(0, 360, 5)
