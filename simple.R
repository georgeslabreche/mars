  ...
  Ls_seq = c(Ls_VE, Ls_A, Ls_SS, Ls_AE, Ls_WS, Ls_P)
  ...
  f = function(Z, tau){
    nnff[toString(tau), paste("X", Z, sep="")]
  }
  ...
  ###
  # Variation of global irradiance on Mars horizontal surface with sun zenith angle and areocentric longitude for different optical depths.
  ###
  # par(mfrow=c(4,3))
  # for(tau in taus){
  #   color_index = 1
  ...
  # }
  # 
  # mtext("Variation of global irradiance on Mars horizontal surface\nwith sun zenith angle and areocentric longitude for different optical depths", side = 3, line = -3, outer = TRUE)
  # 
  # # Add a legend
  # plot.new()
  # legend("top", title="Zenith Angles",
  #        paste("Z = ", zenith_angles, "°", sep=""),
  #        col = wes_palette("FantasticFox1", length(zenith_angles), type = "continuous"),
  #        cex=0.8, bty="n", lty=1)
  
  ###
  # Variation of global irradiance on Mars horizontal surface with optical depth and areocentric longitude for different sun zenith angles.
  ###
  # par(mfrow=c(4,3))
  ...
  #        cex=0.8, bty="n", lty=1)
  # 
  # mtext("Variation of global irradiance on Mars horizontal surface\nwith optical depth and areocentric longitude for different sun zenith angles", side = 3, line = -3, outer = TRUE)
  
  ###
  # Title: Variation of global irradiance on Mars horizontal surface with optical depth and sun zenith angle for different areocentric latitudes.
  ...
  legend("top", title="Optical Depth",
         ...
         col=wes_palette("FantasticFox1", length(taus), type = "continuous"),
         cex=0.8, bty="n", lty=1)
  mtext("Variation of global irradiance on Mars horizontal surface\nwith optical depth and sun zenith angle for different areocentric latitudes.", side = 3, line = -3, outer = TRUE)
